...
export const importGithubRepo = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const internalKey = process.env.CURSOR_CONVEX_INTERNAL_KEY;
    if (!internalKey) {
      throw new NonRetriableError("CURSOR_CONVEX_INTERNAL_KEY is not set");
    }

    const octokit = new Octokit({ auth: githubToken });

    // Cleanup any existing files in the project before importing new ones
    await step.run("cleanup-peoject", async () => {
